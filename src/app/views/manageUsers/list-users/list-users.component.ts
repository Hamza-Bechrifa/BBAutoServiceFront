...
export class ListUsersComponent implements OnInit {
  ...
  @ViewChild('activateModal',{static: false}) activateModal;
  @ViewChild('desactivateModal',{static: false}) desactivateModal;
  ...
  delete(){
    this.userService.deleteUser(this.selectedUser.id).subscribe(
      ()=>{
        ...
        if(this.lang=='fr'){
          this.toasterService.pop('info', '', 'Utilisateur supprimer')}
          else{
            this.toasterService.pop('info', '', 'User deleted')
          }
        ...
        (err) => {
          ...
          this.deleteModal.hide();
          this.toasterService.pop('error', '', err.value)
        }
      ...
      this.userService.update(this.selectedUser.id,form).subscribe(
        (resp:any) => {
          ...
          this.users.find(c=>c.id==this.selectedUser.id).phoneNumber=form.phoneNumber;
            ...
            if(this.lang=='fr'){
              this.toasterService.pop('info', '', 'l\'utilisateur a été modifié avec succes');}
              else{
                this.toasterService.pop('info', '', 'User has been successfully modified');
              }
